...
1. **Compute Mode**: This mode uses the master password combined with the username and domain to create "computed" passwords (similar to [lesspass]). In this mode, only a "password profile" is stored on the server, not the password itself. This mode is used when the 'Compute/Save' button is used.
